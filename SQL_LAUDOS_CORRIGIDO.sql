...
-- Criar políticas de segurança (corrigidas)
CREATE POLICY "Users can view own laudos" ON laudos FOR SELECT USING (classifier_id = auth.uid());
...
CREATE POLICY "Users can update own laudos" ON laudos FOR UPDATE USING (classifier_id = auth.uid());
